...
function CNetworkManager(){
    ...
    this._setValidNick = function(szNickname){
        ...
        szValidNickname = this._stripHTML(szValidNickname);
    ...
    */
    ...
    this.tryCreateUniqueRoom = function(szRoomID, szPass){
        _szCurRoomID = s_oNetworkManager._stripHTML(szRoomID);
    ...
    this._extractRoomInfo = function(aRooms){
        ...
        for(var i=0; i<aRooms.length; i++){
            if(!this._detectHTML(aRooms[i].id)){
    ...
    this._stripHTML = function(myString){
        return myString.replace(/<[^>]*>?/gm, '');
    };

    this._detectHTML = function(myString){
        var szNewString = myString.replace(/<[^>]*>?/gm, '');
        
        return szNewString !== myString;
    };
