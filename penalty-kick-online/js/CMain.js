...
*/
        ...
        s_oSpriteLibrary.addSprite("bg_game_striker", "./sprites/striker/bg_game_striker.jpg");
        s_oSpriteLibrary.addSprite("goal_0", "./sprites/striker/goal_0.png");
        //s_oSpriteLibrary.addSprite("gk_idle_0_0", "./sprites/gk_idle_0_0.png");
        ...
        s_oSpriteLibrary.addSprite("selection_gender", "./sprites/selection_gender.png");
    ...
    this.getGenderSprites = function(szGender){
        var aResources = new Array();

        //PLAYER SPRITES
        for(var t=0;t<PLAYER_PIECES.length;t++){
            for(var i=0;i<PLAYER_PIECES[t].variants;i++){
                
                ///STRIKER ONLY MALE
                var szMale = GENDER_MALE;
                
                for(var k=0;k<PLAYER_INFOS[szMale][PLAYER_PIECES[t].type].num_spritesheet;k++){
                    aResources.push({name:szMale+"_player_"+PLAYER_PIECES[t].type+"_"+i+"-"+k, path:
                            "./sprites/striker/player/"+szMale+"/"+PLAYER_PIECES[t].type+"/"+szMale+"_player_"+PLAYER_PIECES[t].type+"_"+i+"-"+k+".png"});
                }
            }
        }
        
        //OPPONENT
        for(var t=0;t<OPPONENT_PIECES.length;t++){
            for(var i=0;i<OPPONENT_PIECES[t].variants;i++){
                aResources.push({name:szGender+"_opponent_"+OPPONENT_PIECES[t].type+"_"+i,path:
                            "./sprites/goalkeeper/opponent/"+szGender+"/"+OPPONENT_PIECES[t].type+"/"+szGender+"_opponent_"+OPPONENT_PIECES[t].type+"_"+i+".png"});

            }
        } 
        
        //GOALKEEPER ANIMATIONS
        for(var i=0;i<SPRITE_NAME_GOALKEEPER.length;i++){
            for(var j=0;j<GOALKEEPER_PIECES.length;j++){
                //MALE GOALKEEPER
                for(var k=0;k<GOALKEEPER_ANIM_INFOS[szGender][SPRITE_NAME_GOALKEEPER[i]][GOALKEEPER_PIECES[j]].num_variants;k++){
                    
                    aResources.push({name:szGender+"_"+SPRITE_NAME_GOALKEEPER[i]+"_"+GOALKEEPER_PIECES[j]+"-"+k, path:
                                           "./sprites/striker/goalkeeper/"+szGender+"/"+SPRITE_NAME_GOALKEEPER[i]+"/"+szGender+"_"+
                                                            SPRITE_NAME_GOALKEEPER[i]+"_"+GOALKEEPER_PIECES[j]+"-"+k+".png"});
                }
            }
        }
    ...
    /*
    //TEST
    AREAS_INFO = [  {id: 0, probability: 0}, {id: 1, probability: 0}, {id: 2, probability: 0},{id: 3, probability: 0}, {id: 4, probability: 0}, 
                    {id: 5, probability: 0}, {id: 6, probability: 0}, {id: 7, probability: 0}, {id: 8, probability: 0}, {id: 9, probability: 0}, 
                    {id: 10, probability: 0}, {id: 11, probability: 0},{id: 12, probability: 0}, {id: 13, probability: 0}, {id: 14, probability: 0}]; 
   */
